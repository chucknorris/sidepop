...
namespace sidepop.Mail.Commands
{
    ...
    internal abstract class Pop3Command<T> : Command, IDisposable where T : Pop3Response
    {
        ...
        private byte[] GetResponse()
        {
            ...
            if (IsMultiline)
            {
                callback = GetMultiLineResponseCallback;
            }
            else
            {
                callback = GetSingleLineResponseCallback;
            }
            try
            {
                Receive(callback);
        ...
        private void GetSingleLineResponseCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                {
                    Receive(GetSingleLineResponseCallback);
                }

            }
            catch (Exception ex)
            {
                //for now we swallow - network problems cause this to be an issue
                Log.bound_to(this).log_an_error_event_containing("{0} encountered an error during single line callback:{1}{2}", ApplicationParameters.name, Environment.NewLine, ex.ToString());
                _manualResetEvent.Set();
            }
        }
        ...
        protected virtual byte[] GetRequestMessage(params string[] args)
        {
            string message = string.Join(string.Empty, args);
            Log.bound_to(this).log_a_debug_event_containing(message);
