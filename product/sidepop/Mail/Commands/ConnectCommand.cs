		...
		private void SetClientStream()
		{
			if (_useSsl)
			{
				try
				{
					...
					//make sure the inner stream stays available for the DefaultPop3Client to make use of.
				...
				catch (InvalidOperationException e)
				{
					throw new Pop3Exception(
						"There was a problem attempting to authenticate this SSL stream for hostname: " + _hostname, e);
				}
				catch (Exception e)
				{
				    Log.bound_to(this).log_a_warning_event_containing("There was an error during connection: {0}{1}",Environment.NewLine, e.ToString());
				}
